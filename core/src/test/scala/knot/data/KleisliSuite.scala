...
object KleisliSuite extends SimpleIOSuite with Discipline {
  ...
  checkAll("Kleisli[List, MiniInt, *]", MonoidKTests[Kleisli[List, MiniInt, *]].monoidK[Int])
  ...
  checkAll("Kleisli[Option, *, *]", ChoiceTests[Kleisli[Option, *, *]](Kleisli.choiceForKleisli).choice[MiniInt, Boolean, Int, Int])
  ...
  checkAll("Kleisli[Option, *, *]", SemigroupKTests[Kleisli[Option, MiniInt, *]].semigroupK[MiniInt])
  ...
  pureTest("Kleisli[Option, MiniInt, *]: monoid.empty") {
    val fa = Monoid[Kleisli[Option, String, Int]].empty
    expect.same(fa.run("12"), None)
  }
  pureTest("mapFilter") {
    val fa = Kleisli
