...
import knot.util.CatsEffectInstances.given
...
object SerializerSuite extends SimpleIOSuite with Discipline {
  ...
  pureTest("map") {
    val fa = Serializer
      ...
      .map(_ + 1)
    expect.same(fa.run(List(1, 2, 3)), Stream(2, 3, 4))
  }
  pureTest("dimap") {
    val fa = Serializer
      .instance[Id, List[Int], Int](Stream.emits)
      .dimap[Int, String](i => List.fill(i)(1))(_.toString)
    expect.same(fa.run(3), Stream("1", "1", "1"))
  }
  pureTest("lmap") {
    val fa = Serializer
      .instance[Id, List[Int], Int](Stream.emits)
  ...
  pureTest("andThen") {
    ...
    val fe = fa >>> fb
    expect.same(fc.run(List(1, 2, 3)), Stream("1", "2", "3")) and
      expect.same(fd.run(List(1, 2, 3)), Stream("1", "2", "3")) and
      expect.same(fe.run(List(1, 2, 3)), Stream("1", "2", "3"))
  }
  pureTest("compose") {
    val fa = Serializer.instance[Id, Int, String](i => Stream(i.toString))
    val fb = Serializer.instance[Id, List[Int], Int](Stream.emits)
    val fc = fa <<< fb
    expect.same(fc.run(List(1, 2, 3)), Stream("1", "2", "3"))
  }
  pureTest("second") {
    val fa = Serializer
      .instance[Id, List[Int], Int](Stream.emits)
      .second[Char]
    expect.same(fa.run('c' -> List(1, 2, 3)), Stream('c' -> 1, 'c' -> 2, 'c' -> 3))
  }
